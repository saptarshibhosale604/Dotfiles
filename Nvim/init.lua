...
require("lazy").setup({
	...
	-- options to `gitsigns.nvim`. this is good
	--
...

	{
		...
		opts = {
			...
			providers = {
				gemini = {
					extra_request_body = {
						-- model = "gemini-1.5-pro-exp-0827", -- or another supported model
						model = "gemini-1.5-pro", -- or another supported model
						temperature = 0,
						max_tokens = 4096,
...

	{
		...
		config = function()
			require("focus").setup({
				autoresize = {
					enable = true,
					width = 100, -- Set focused window width to 120 cols (customize as needed)
					minwidth = 50, -- Optional: minimum width for unfocused windows
				},
			})
		end,
	},
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		config = function()
			...
			-- vim.keymap.set("n", "<leader>to", ":tabnew %<CR >", { desc = "[T]ab [O]pen current buffer" })
	...
	{ -- Collection of various small independent plugins/modules
		...
		config = function()
			...
			-- Function to get current git branch name (returns empty string if no branch)
			...
			statusline.section_location = function()
				local clock = os.date("%H:%M") -- 24-hour format HH:MM
				local branch = get_git_branch()
				if branch ~= "" then
					branch = "  " .. branch -- add branch symbol and space to differentiate
				end
				return "%2l:%-2v " .. clock .. branch
			end
			-- ... and there is more!
